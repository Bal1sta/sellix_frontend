...
export default function ProducerCabinet() {
  ...
  // Если профиль компании не создан — отправляем на страницу создания
  useEffect(() => {
    producerApi.profile.get()
      .then(() => setReady(true))
      .catch(() => navigate("/producer/create", { replace: true }));
  }, [navigate]);
  ...
  return (
    <CabinetShell title="Кабинет производителя" tabs={TABS} activeTab={tab} onTab={setTab}>
...
// --- Профиль компании (только редактирование — создание на /producer/create) ---
function CompanyTab() {
